    ...
    func signUp(){
        ...
        Auth.auth().createUser(withEmail: email_SignUp, password: password_SignUp) { (result, err) in
            ...
            if let user = user {
                ...
                db.collection("RiderProfiles").document(uid).setData(["Age": 0, "Email": email ?? "", "Gender": 0, "Height": 0, "ID": uid, "Joined Date": formatter1.string(from: date), "Owned Horse": "", "Phone": phone, "name": name, "Horse Name": "", "Instructor": "", "Instructor Name": ""], merge:true)
