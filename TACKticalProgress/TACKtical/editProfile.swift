        ...
        Form {
            Section {
                ...
                HStack() {
                    ...
                    TextField("Enter new owner", text: $horse.owner)
                }
...
//            }.frame(width:UIScreen.main.bounds.width*0.3, height:UIScreen.main.bounds.height*0.035, alignment:.center).foregroundColor(.black).background(Color(UIColor.lightGray)).opacity(0.7).cornerRadius(16).padding(UIScreen.main.bounds.height*0.005)
    ...
    func upload() {
        ...
        db.collection("HorseProfiles").document(id).setData(["Arrival Date": formatter1.string(from: horse.arrivalDate), "Color": horse.color, "Date of Birth": formatter1.string(from: horse.birth), "Feed": horse.feed, "Gender": horse.gender, "Height": horse.height, "Owner": horse.owner, "name":horse.name], merge:true)
    }
